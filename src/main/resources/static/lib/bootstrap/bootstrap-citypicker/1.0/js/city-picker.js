...
/*
    ...
    CityPicker.prototype = {
        ...
        loadData: function () {
            var that = this;
            if (that.options.dataSource == 'server') {
                $.ajax({
                    ...
                    success: function (res) {
                        ChineseDistricts = that.options.dataHandler && that.options.dataHandler(res);
                    }
                });
            }
        },
        ...
        render: function () {
            var p = this.getPosition(),
                ...
                textspan = '<span class="city-picker-span ' + this.options.pickerClass + '" style="' +
        ...
        bind: function () {
            ...
            if (this.$province) {
                this.$province.on(EVENT_CHANGE, (this._changeProvince = $.proxy(function () {
                    ...
                    $parent.find('input.city_picker_province').val(this.$province.find('.active').data('code'));
                    $parent.find('input.city_picker_city').val('');
                    $parent.find('input.city_picker_district').val('');
            ...
            if (this.$city) {
                this.$city.on(EVENT_CHANGE, (this._changeCity = $.proxy(function () {
                    ...
                    $parent.find('input.city_picker_city').val(this.$city.find('.active').data('code'));
                    $parent.find('input.city_picker_district').val('');
            ...
            if (this.$district) {
                this.$district.on(EVENT_CHANGE, (this._changeDistrict = $.proxy(function () {
                    $parent.find('input.city_picker_district').val(this.$district.find('.active').data('code'));
                }, this)));
            }
        },
        ...
        echo: function (data) {
            ...
            if (data) {
                for (var p in data) {
                    that.options[p] = data[p];
                    that.output(p);
                    //临时增加处理赋值
                    $parent.find('input.city_picker_' + p).val(data[p]);
                }
                that.refresh();
            }
        },
